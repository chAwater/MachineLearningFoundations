...
- 拿出来的 _N_ 个球（Sample） **相当于** 机器学习的数据集
...
那么怎么办？Hoeffding不等式！它也保证了对于某一个`假设函数`，数据集中出现导致`假设函数`和`目标函数`出现差距的数据（“不好的”）**很少**。
...
当 _k_ < _N_ 时，（这是我们比较关心的情况）可将 _B_(_N_, _k_) 个 Dichotomies 分为两类：
