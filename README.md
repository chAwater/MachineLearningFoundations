...
通常的问题都是多类别的分类，而不是二元分类；因此我们想要把我们已经熟悉的二元分类问题延伸到多类别分类问题中。

一个简单的想法是：先把其中这些类别中的一个类别先分出来，把其他的类别当做另外一个类别，这样就把这个问题转换成一个二元分类问题；然后再选择另外一个类别，进行类似的操作；

但是这样做有一些问题：
- 有些数据被多个二元分类器判断为某个分类
- 或者被所有的分类器排除

下面一个解决的思路就是用 soft 二元分类，算出每个点属于某个类别的可能性。这样最后每个数据选择可能性最大的那个分类就能够解决上面的两个问题。因为 soft 二元分类（逻辑回归）中的 <i>&theta;</i> 是单调的，因此可以直接比较取 <i>&theta;</i> 之前的值，因此有：
...
对于OVA中的不平衡问题，一个简单的思路是从所有的分类中任选两个分类，来做二元分类。如果每个分类在数据中是比较平均的，那么这样就可以避免不平衡的问题。

那么最后如何整合这些分类的结果呢？用投票的方法！
