...
当 _k_ = _N_ 时，_B_(_N_, _k_) = 2<sup>_N_</sup>-1；（需要有一种情况不满足，否则就是 Shatter ）
...
- 为了证明 1，我们只需要找到一组数据，有 _d_+1 个，并且能够 Shatter ：

> 有这样一个矩阵 X （_d_+1 x _d_+1），它是可逆的（Col都是独立的）；
>
...
- 为了证明 2，我们需要证明对于 **任何** _d_+2 个输入，都不能 Shatter：

> Shatter 代表这些输入是没有线性相关关系的
>
> 对于这种矩阵 X（_d_+2 x _d_+1），其 **行数多于列数**，因此**行**存在 **线性相关** 关系；
>
> 因此有（线性组合）：
>
...
思考：为什么对于矩阵 X，因为其 **行数多于列数**，所以存在 **线性相关** 关系？

参见：[Issues #2](https://github.com/chAwater/MachineLearningFoundations/issues/2)
...
但是，<i>E</i><sub>in</sub> 肯定是很小的，如果 <i>E</i><sub>out</sub> 也很小，那么就可是算是 **学习** 到了。

而且当计算这个 pseudo-inverse 的时候，其实也可以看做是不断改进的过程，<i>E</i><sub>in</sub> 也是在不断的变好的。
...
自由度为 _N_ 的向量，投影到 _d_ + 1 维的空间时，剩下的自由度最多只有 _N_ - ( _d_ + 1 )。

下面看一下 <i>E</i><sub>in</sub> 的均值和这些的关系：

- 我们可以把 y 看成一个理想的函数 _f_(X)，加上一些`噪音`（noise）
...
请在 [Issues #5](https://github.com/chAwater/MachineLearningFoundations/issues/5) 中回答。
...
在这里，我们用`上限的上限`限制住了 <i>E</i><sub>out</sub>，用一个`宽松`的限制来换取更高的计算`效率`。
...
[Issues #6](https://github.com/chAwater/MachineLearningFoundations/issues/6)

---

### 梯度 (Gradient)

可以证明逻辑回归的 <i>E</i><sub>in</sub> 是`连续的`、`可微分的`、`凸的`；所以可以用和线性回归类似的方法，算出它的梯度，从而找到最小值。
...
这样一来，我们可能得到一条四次曲线，这条曲线的 <i>E</i><sub>in</sub> 很小，甚至是 0；

但是这个曲线在我们没有看到的数据上的表现肯定很差，也就是说 <i>E</i><sub>out</sub> 会很大；
...
也就是说，通过增加 Model Complexity ，我们把 fitting 做得更好了，但是却做得越来越过头（over）了，导致 <i>E</i><sub>out</sub> 的增加；
...
Bad Generalization 是指在 Model Complexity 曲线中一个点的状态，<i>E</i><sub>in</sub> 很小但 <i>E</i><sub>out</sub> 很大；

而 Overfitting 是指一个 <i>E</i><sub>in</sub> 变低但<i>E</i><sub>out</sub> 却变高的过程。
...
对于第一个例子，即使我们知道`目标函数`是一个十次多项式，我们仍然是用二次多项式能做得更好。以退为进！这是问什么？
...
假设我们有8年的汇率数据，我们用前6年做训练，后2年做测试来预测汇率的变化。但是在我们训练模型之前需要把数据做一个`标准化`的过程。
...
这相当于什么？我们在讨论`模型选择`时说过，这相当于把 模型1、2、3、4 放在一起选择（甚至还包含那些没有发表的不好的模型！），然后找出最好的！<i>d</i><sub>VC</sub> 很大！
...
偷看数据非常难以避免，一些好的方法有：
- 使用 **验证**
- 避免用数据来做决定/选择，不要看完数据再做决定/选择，但可以先做决定/选择再看数据
- 永远保持怀疑
...
<!-- Last Update 2019.04.13 20:12:31 -->
